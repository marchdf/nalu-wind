...
ScalarTAMSUpwAdvDiffElemKernel<AlgTraits>::ScalarTAMSUpwAdvDiffElemKernel(
    ...
    pecletFunction_(eqSystem->create_peclet_function<DoubleType>(dofName_))
{
  ...
  const std::string vrtm_name =
    solnOpts.does_mesh_move() ? "average_velocity_rtm" : "average_velocity";
  ...
  get_scs_shape_fn_data<AlgTraits>(
    [&](double* ptr) {
      skewSymmetric ? meSCS->shifted_shape_fcn(ptr) : meSCS->shape_fcn(ptr);
    },
...
template <typename AlgTraits>
void
ScalarTAMSUpwAdvDiffElemKernel<AlgTraits>::setup(const TimeIntegrator&)
{
...
template <typename AlgTraits>
void
ScalarTAMSUpwAdvDiffElemKernel<AlgTraits>::execute(
  ...
  ScratchViews<DoubleType>& scratchViews)
{
  ...
  SharedMemView<DoubleType**>& v_velocityRTM =
    scratchViews.get_scratch_view_2D(velocityRTM_);
  ...
  SharedMemView<DoubleType*>& v_scalarQ =
    scratchViews.get_scratch_view_1D(scalarQ_);
  SharedMemView<DoubleType*>& v_density =
    scratchViews.get_scratch_view_1D(density_);
  SharedMemView<DoubleType*>& v_diffFluxCoeff =
    scratchViews.get_scratch_view_1D(diffFluxCoeff_);
  SharedMemView<DoubleType*>& v_mdot =
    scratchViews.get_scratch_view_1D(massFlowRate_);
  ...
  for (int ip = 0; ip < AlgTraits::numScsIp_; ++ip) {
    ...
    // upwind
    const DoubleType qUpwind = stk::math::if_then_else(
      tmdot > 0, alphaUpw_ * qIpL + om_alphaUpw_ * qIp,
    ...
    const DoubleType alhsfacL =
      0.5 * (tmdot + stk::math::abs(tmdot)) * pecfac * alphaUpw_ +
      0.5 * alpha_ * om_pecfac * tmdot;
    lhs(il, il) += alhsfacL;
    lhs(ir, il) -= alhsfacL;

    // upwind advection; right node
    const DoubleType alhsfacR =
      0.5 * (tmdot - stk::math::abs(tmdot)) * pecfac * alphaUpw_ +
      0.5 * alpha_ * om_pecfac * tmdot;
    lhs(ir, ir) -= alhsfacR;
    lhs(il, ir) += alhsfacR;
    ...
    for (int ic = 0; ic < AlgTraits::nodesPerElement_; ++ic) {
      ...
      const DoubleType lhsfacAdv =
        v_adv_shape_function_(ip, ic) * tmdot *
        (pecfac * om_alphaUpw_ + om_pecfac * om_alpha_);
...
DoubleType
ScalarTAMSUpwAdvDiffElemKernel<AlgTraits>::van_leer(
  const DoubleType& dqm, const DoubleType& dqp)
{
...
INSTANTIATE_KERNEL(ScalarTAMSUpwAdvDiffElemKernel)
