...
MomentumTAMSKEpsForcingElemKernel<
  AlgTraits>::~MomentumTAMSKEpsForcingElemKernel()
{
  tmpFile.close();
}
  ...
  for (int ip = 0; ip < AlgTraits::numScvIp_; ++ip) {
    ...
    if ((step_ % 100000) == 0) {
      tmpFile << w_coordScs[0] << w_coordScs[1] << w_coordScs[2] << gX << gY
              << gZ << norm << F_target << alphaScs << prod_r << arg << fd_temp
              << Sa << a_kol << a_sign << std::endl;
    }

    // if ((step_ % 1) == 0)
    //{
    //  tmpFile << w_coordScs[0] << w_coordScs[1] << w_coordScs[2] << gX << gY
    //  << gZ << norm << alphaScs << v2Scs << tkeScs << tdrScs << fd_temp << hX
    //  << hY << hZ << std::endl;
    //}
