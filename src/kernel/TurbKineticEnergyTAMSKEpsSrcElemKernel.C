...
template <typename AlgTraits>
TurbKineticEnergyTAMSKEpsSrcElemKernel<AlgTraits>::~TurbKineticEnergyTAMSKEpsSrcElemKernel()
{
  tmpFile.close();
}
  ...
  ScratchViews<DoubleType>& scratchViews)
{
  ...
  for (int ip = 0; ip < AlgTraits::numScvIp_; ++ip) {
    ...
    //if ((step_ % 1) == 0)
    //{
    //  tmpFile << w_coordScs[0] << w_coordScs[1] << w_coordScs[2] << Pk << Dk << Lk << visc << minD << tke << tdr << std::endl;
    //}
