  ...
  : Algorithm(realm, part)
{
  ...
  totalDissRate_ = meta_data.get_field<ScalarFieldType>(
...
//--------------------------------------------------------------------------
void
ComputeTAMSKEpsKratioNodeAlgorithm::execute()
{
  ...
  for (stk::mesh::BucketVector::const_iterator ib = node_buckets.begin();
    ...
    for (stk::mesh::Bucket::size_type k = 0; k < length; ++k) {
      if (tke[k] == 0.0)
        ...
        const double T_ke = avgTime[k]; // tke[k] / std::max(tdr[k],1e-16);
        ...
        const double a_kol = std::min(
          1.5 * v2 / tke[k] * std::sqrt(visc[k] * tdr[k]) / tke[k], 1.0);
