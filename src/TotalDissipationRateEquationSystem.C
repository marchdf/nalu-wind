    ...
    diffFluxCoeffAlgDriver_(new AlgorithmDriver(realm_))
{
  ...
  NaluEnv::self().naluOutputP0() << "Edge projected nodal gradient for total_dissipation_rate: " << edgeNodalGradient_ <<std::endl;
  ...
  if ( managePNG_ )
    throw std::runtime_error("TotalDissipationRateEquationSystem::Error managePNG is not complete");
}
  ...
  stk::mesh::Part *part)
{
  ...
  evisc_ = &(meta_data.declare_field<ScalarFieldType>(stk::topology::NODE_RANK, "effective_viscosity_tdr"));
  ...
  stk::mesh::Part *part)
{
  ...
  if ( it == assembleNodalGradAlgDriver_->algMap_.end() ) {
    ...
    if ( edgeNodalGradient_ && realm_.realmUsesEdges_ ) {
      theAlg = new AssembleNodalGradEdgeAlgorithm(realm_, part, &tdrNp1, &dedxNone);
    }
    else {
      theAlg = new AssembleNodalGradElemAlgorithm(realm_, part, &tdrNp1, &dedxNone, edgeNodalGradient_);
    }
  ...
  if (!realm_.solutionOptions_->useConsolidatedSolverAlg_) {
    ...
    if (itsi == solverAlgDriver_->solverAlgMap_.end()) {
      ...
      std::map<std::string, std::vector<std::string> >::iterator isrc
        = realm_.solutionOptions_->elemSrcTermsMap_.find("total_dissipation_rate");
      if (isrc != realm_.solutionOptions_->elemSrcTermsMap_.end()) {

        if (realm_.realmUsesEdges_)
          throw std::runtime_error("TotalDissipationElemSrcTerms::Error can not use element source terms for an edge-based scheme");
        ...
        for (size_t k = 0; k < mapNameVec.size(); ++k) {
          ...
          if (sourceName == "NSO_2ND_ALT") {
            suppAlg = new ScalarNSOElemSuppAlgDep(realm_, tdr_, dedx_, evisc_, 0.0, 1.0);
          }
          else if (sourceName == "NSO_4TH_ALT") {
            suppAlg = new ScalarNSOElemSuppAlgDep(realm_, tdr_, dedx_, evisc_, 1.0, 1.0);
          }
          else if (sourceName == "NSO_2ND_KE") {
            const double turbSc = realm_.get_turb_schmidt(tdr_->name());
            suppAlg = new ScalarNSOKeElemSuppAlg(realm_, tdr_, dedx_, turbSc, 0.0);
          }
          else if (sourceName == "NSO_4TH_KE") {
            const double turbSc = realm_.get_turb_schmidt(tdr_->name());
            suppAlg = new ScalarNSOKeElemSuppAlg(realm_, tdr_, dedx_, turbSc, 1.0);
          }
          else if (sourceName == "total_dissipation_rate_time_derivative" ) {
            suppAlg = new ScalarMassElemSuppAlgDep(realm_, tdr_, false);
          }
          else if (sourceName == "lumped_total_dissipation_rate_time_derivative" ) {
            suppAlg = new ScalarMassElemSuppAlgDep(realm_, tdr_, true);
          }
          else {
            throw std::runtime_error("TotalDissipationElemSrcTerms::Error Source term is not supported: " + sourceName);
          }
          NaluEnv::self().naluOutputP0() << "TotalDissipationElemSrcTerms::added() " << sourceName << std::endl;
    ...
    if (itsm == solverAlgDriver_->solverAlgMap_.end()) {
      ...
      if (!elementMassAlg) {
        if (realm_.number_of_states() == 2) {
          ScalarMassBackwardEulerNodeSuppAlg *theMass
            = new ScalarMassBackwardEulerNodeSuppAlg(realm_, tdr_);
          theAlg->supplementalAlg_.push_back(theMass);
        }
        else {
          ScalarMassBDF2NodeSuppAlg *theMass
            = new ScalarMassBDF2NodeSuppAlg(realm_, tdr_);
          theAlg->supplementalAlg_.push_back(theMass);
        }
      }
      ...
      std::map<std::string, std::vector<std::string> >::iterator isrc
        = realm_.solutionOptions_->srcTermsMap_.find("total_dissipation_rate");
      if (isrc != realm_.solutionOptions_->srcTermsMap_.end()) {
        ...
        for (size_t k = 0; k < mapNameVec.size(); ++k) {
          ...
          SupplementalAlgorithm* suppAlg = NULL;
          if (sourceName == "gcl") {
            suppAlg = new ScalarGclNodeSuppAlg(tdr_, realm_);
          }
          else {
            throw std::runtime_error("TotalDissipationRateNodalSrcTerms::Error Source term is not supported: " + sourceName);
          }
          NaluEnv::self().naluOutputP0() << "TotalDissipationRateNodalSrcTerms::added() " << sourceName << std::endl;
          theAlg->supplementalAlg_.push_back(suppAlg);
        }
      }
    }
    ...
    if (realm_.realmUsesEdges_)
      throw std::runtime_error("TotalDissipationRateEquationSystem::Error can not use element source terms for an edge-based scheme");
    ...
    if (solverAlgWasBuilt) {
      build_topo_kernel_if_requested<ScalarMassElemKernel>
        (partTopo, *this, activeKernels, "total_dissipation_rate_time_derivative",
         realm_.bulk_data(), *realm_.solutionOptions_, tdr_, dataPreReqs, false);

      build_topo_kernel_if_requested<ScalarMassElemKernel>
        (partTopo, *this, activeKernels,  "lumped_total_dissipation_rate_time_derivative",
         realm_.bulk_data(), *realm_.solutionOptions_, tdr_, dataPreReqs, true);
      ...
      build_topo_kernel_if_requested<ScalarNSOElemKernel>
        (partTopo, *this, activeKernels, "NSO_2ND",
         realm_.bulk_data(), *realm_.solutionOptions_, tdr_, dedx_, evisc_, 0.0, 0.0, dataPreReqs);

      build_topo_kernel_if_requested<ScalarNSOElemKernel>
        (partTopo, *this, activeKernels, "NSO_2ND_ALT",
         realm_.bulk_data(), *realm_.solutionOptions_, tdr_, dedx_, evisc_, 0.0, 1.0, dataPreReqs);

      build_topo_kernel_if_requested<ScalarNSOElemKernel>
        (partTopo, *this, activeKernels, "NSO_4TH",
         realm_.bulk_data(), *realm_.solutionOptions_, tdr_, dedx_, evisc_, 1.0, 0.0, dataPreReqs);
  ...
  if ( itev == diffFluxCoeffAlgDriver_->algMap_.end() ) {
    ...
    EffectiveDiffFluxCoeffAlgorithm *effDiffAlg
      = new EffectiveDiffFluxCoeffAlgorithm(realm_, part, visc_, tvisc_, evisc_, 1.0, sigmaEps);
  ...
  const InflowBoundaryConditionData &inflowBCData)
{
  ...
  if ( it == assembleNodalGradAlgDriver_->algMap_.end() ) {
    Algorithm *theAlg 
      = new AssembleNodalGradBoundaryAlgorithm(realm_, part, &tdrNp1, &dedxNone, edgeNodalGradient_);
    assembleNodalGradAlgDriver_->algMap_[algType] = theAlg;
  }
  else {
    it->second->partVec_.push_back(part);
  ...
  const OpenBoundaryConditionData &openBCData)
{
  ...
  // register boundary data; tdr_bc
  ScalarFieldType *theBcField = &(meta_data.declare_field<ScalarFieldType>(stk::topology::NODE_RANK, "open_tdr_bc"));
  stk::mesh::put_field_on_mesh(*theBcField, *part, nullptr);
  ...
  if ( itsi == solverAlgDriver_->solverAlgMap_.end() ) {
    SolverAlgorithm *theAlg = NULL;
    if ( realm_.realmUsesEdges_ ) {
      theAlg = new AssembleScalarEdgeOpenSolverAlgorithm(realm_, part, this, tdr_, theBcField, &dedxNone, evisc_);
    }
    else {
      theAlg = new AssembleScalarElemOpenSolverAlgorithm(realm_, part, this, tdr_, theBcField, &dedxNone, evisc_);
    }
    solverAlgDriver_->solverAlgMap_[algType] = theAlg;
  }
  else {
  ...
  const stk::topology &/*theTopo*/)
{
  ...
  if ( edgeNodalGradient_ ) {    
    ...
    if ( it == assembleNodalGradAlgDriver_->algMap_.end() ) {
      AssembleNodalGradNonConformalAlgorithm *theAlg 
        = new AssembleNodalGradNonConformalAlgorithm(realm_, part, &tdrNp1, &dedxNone);
      assembleNodalGradAlgDriver_->algMap_[algType] = theAlg;
    }
    else {
      it->second->partVec_.push_back(part);
  ...
  if ( itsi == solverAlgDriver_->solverAlgMap_.end() ) {
    AssembleScalarNonConformalSolverAlgorithm *theAlg
      = new AssembleScalarNonConformalSolverAlgorithm(realm_, part, this, tdr_, evisc_);
    solverAlgDriver_->solverAlgMap_[algType] = theAlg;
  }
  else {
    itsi->second->partVec_.push_back(part);
...
TotalDissipationRateEquationSystem::predict_state()
{
  ...
  ScalarFieldType &tdrN = tdr_->field_of_state(stk::mesh::StateN);
  ScalarFieldType &tdrNp1 = tdr_->field_of_state(stk::mesh::StateNP1);
  field_copy(realm_.meta_data(), realm_.bulk_data(), tdrN, tdrNp1, realm_.get_activate_aura());
}
