...
//#include "ngp_algorithms/TDRWallFuncAlg.h"
//#include "ngp_algorithms/TDRLowReWallAlg.h"
//#include "ngp_algorithms/TDRWallFuncAlgDriver.h"
  ...
  if (!realm_.solutionOptions_->useConsolidatedSolverAlg_) {
    ...
    process_ngp_node_kernels(
      ...
      [&](AssembleNGPNodeSolverAlgorithm& nodeAlg) {
        ...
        if (KE == realm_.solutionOptions_->turbulenceModel_) {
          nodeAlg.add_kernel<TDRKENodeKernel>(realm_.meta_data());
        } else if (KE_AMS == realm_.solutionOptions_->turbulenceModel_)
  ...
  const WallBoundaryConditionData& /*wallBCData*/)
{
  ...
  //  // create proper algorithms to fill nodal omega and assembled wall area;
  //  utau managed by momentum if (!wallModelAlgDriver_)
  //    wallModelAlgDriver_.reset(new TDRWallFuncAlgDriver(realm_));

  //  if (wallFunctionApproach || RANSAblBcApproach) {
  //    RoughnessHeight rough = userData.z0_;
  //    double z0 = rough.z0_;
  //    wallModelAlgDriver_->register_face_elem_algorithm<TDRWallFuncAlg>(
  //      algType, part, get_elem_topo(realm_, *part), "tdr_wall_func",
  //      RANSAblBcApproach, z0);
  //  }
  //  else {
  //    wallModelAlgDriver_->register_face_elem_algorithm<TDRLowReWallAlg>(
  //      algType, part, get_elem_topo(realm_, *part), "tdr_wall_func",
...
//--------------------------------------------------------------------------
// void
// TotalDissipationRateEquationSystem::compute_wall_model_parameters()
//{
