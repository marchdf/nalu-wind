...
#include "edge_kernels/MomentumTAMSSSTDiffEdgeSolverAlg.h"
...
MomentumTAMSSSTDiffEdgeSolverAlg::MomentumTAMSSSTDiffEdgeSolverAlg(
    ...
    CMdeg_(realm.get_turb_model_constant(TM_CMdeg))
{
  ...
  avgDudx_ = get_field_ordinal(meta, "average_dudx");
...
void
MomentumTAMSSSTDiffEdgeSolverAlg::execute()
{
    ...
    KOKKOS_LAMBDA(
      ...
      for (unsigned i = 0; i < ndim; i++)
        for (unsigned j = 0; j < ndim; j++)
          // FIXME: Is this right for accessing 2D array of nodal Mij, is it 1D
          // or 2D??
