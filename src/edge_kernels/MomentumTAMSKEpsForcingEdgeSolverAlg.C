...
MomentumTAMSKEpsForcingEdgeSolverAlg::execute()
{
  ...
  run_algorithm(
      ...
      const stk::mesh::FastMeshIndex& nodeR)
    {
      ...
      const DblType minDistIp = 0.5 * (minDist.get(nodeL, 0) + minDist.get(nodeR, 0));
      ...
      // FIXME: Deal with Mij
      const DblType ceilLengthX = stk::math::max(length,  2.0 * v_Mij(0, 0));
      const DblType ceilLengthY = stk::math::max(lengthY, 2.0 * v_Mij(1, 1));
      const DblType ceilLengthZ = stk::math::max(length,  2.0 * v_Mij(2, 2));
      ...
      const DblType xarg = ax * (coords[0] + avgU[0] * time_);
      const DblType yarg = ay * (coords[1] + avgU[1] * time_);
      const DblType zarg = az * (coords[2] + avgU[2] * time_);
      ...
      const DblType prod_r = (F_target*dt_)*(hX*w_fluctU[0] + hY*fluctU[1] + hZ*fluctU[2]);
      ...
      const DblType fd_temp = v_avgResAdeq(0);
