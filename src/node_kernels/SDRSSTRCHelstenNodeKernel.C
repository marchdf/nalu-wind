...
SDRSSTRCHelstenNodeKernel::SDRSSTRCHelstenNodeKernel(
  const stk::mesh::MetaData& meta)
  : NGPNodeKernel<SDRSSTRCHelstenNodeKernel>(),
...
void
SDRSSTRCHelstenNodeKernel::setup(Realm& realm)
{
  ...
  cRC_ = realm.get_turb_model_constant(TM_cRCHelsten);
}

void
SDRSSTRCHelstenNodeKernel::execute(
  ...
  const stk::mesh::FastMeshIndex& node)
{
  ...
  Pk *= tvisc;
  sijMag = stk::math::max(stk::math::sqrt(2.0 * sijMag), 1.0e-16);
  wijMag = stk::math::max(stk::math::sqrt(2.0 * wijMag), 1.0e-16);
  ...
  // Hellsten function for RC
  const DblType Ri = wijMag / sijMag * (wijMag / sijMag - 1);
  const DblType fFour = 1. / (1. + cRC_ * Ri);

  // Production term with appropriate clipping of tvisc
  const DblType Pw = gamma * density * Pk / stk::math::max(tvisc, 1.0e-16);
  const DblType Dw = fFour * beta * density * sdr * sdr;
  const DblType Sw = sigmaD * density * crossDiff / sdr;

  rhs(0) += (Pw - Dw + Sw) * dVol;
  lhs(0, 0) +=
    (2.0 * fFour * beta * density * sdr + stk::math::max(Sw / sdr, 0.0)) * dVol;
}
