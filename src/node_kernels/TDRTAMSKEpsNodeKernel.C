...
TDRTAMSKEpsNodeKernel::
  ...
  : NGPNodeKernel<TDRTAMSKEpsNodeKernel>(),
...
void
TDRTAMSKEpsNodeKernel::setup(Realm& realm)
{
  ...
  const stk::mesh::FastMeshIndex& node)
{
  ...
  const NodeKernelTraits::DblType Re_t =
    rho * tke * tke / visc / stk::math::max(tdr, 1.0e-16);
  const NodeKernelTraits::DblType fTwo =
