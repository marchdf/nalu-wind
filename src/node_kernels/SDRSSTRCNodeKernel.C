...
SDRSSTRCNodeKernel::SDRSSTRCNodeKernel(const stk::mesh::MetaData& meta)
  : NGPNodeKernel<SDRSSTRCNodeKernel>(),
...
void
SDRSSTRCNodeKernel::setup(Realm& realm)
{
  ...
  const stk::mesh::FastMeshIndex& node)
{
  ...
  const DblType Pw =
    fROne * gamma * density * Pk / stk::math::max(tvisc, 1.0e-16);
