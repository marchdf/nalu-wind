...
TKETAMSKEpsNodeKernel::TKETAMSKEpsNodeKernel(
  const stk::mesh::BulkData& bulk, const SolutionOptions& solnOpts)
  : NGPNodeKernel<TKETAMSKEpsNodeKernel>(),
...
void
TKETAMSKEpsNodeKernel::setup(Realm& realm)
{
  ...
  const stk::mesh::FastMeshIndex& node)
{
  ...
  const NodeKernelTraits::DblType lFac =
    2.0 * viscosity_.get(node, 0) / minDist / minDist;
