  ...
  if (!realm_.solutionOptions_->useConsolidatedSolverAlg_) {
    ...
    process_ngp_node_kernels(
      ...
      [&](AssembleNGPNodeSolverAlgorithm& nodeAlg) {
        ...
        else if (SST_RC == realm_.solutionOptions_->turbulenceModel_){
          nodeAlg.add_kernel<SDRSSTRCNodeKernel>(realm_.meta_data());
        }
        else if (SST_RC_HELSTEN == realm_.solutionOptions_->turbulenceModel_){
          nodeAlg.add_kernel<SDRSSTRCHelstenNodeKernel>(realm_.meta_data());
        }
        else {
