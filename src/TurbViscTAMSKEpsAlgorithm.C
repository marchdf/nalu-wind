...
// Class Definition
//==========================================================================
// TurbViscTAMSKEpsAlgorithm - compute tvisc for Chien K-epsilon model
//==========================================================================
//--------------------------------------------------------------------------
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
TurbViscTAMSKEpsAlgorithm::TurbViscTAMSKEpsAlgorithm(
    ...
    avgTime_(NULL)
{
  ...
  tdr_ = meta_data.get_field<ScalarFieldType>(
    stk::topology::NODE_RANK, "total_dissipation_rate");
  dplus_ = meta_data.get_field<ScalarFieldType>(
...
//--------------------------------------------------------------------------
void
TurbViscTAMSKEpsAlgorithm::execute()
{
  ...
  for (stk::mesh::BucketVector::const_iterator ib = node_buckets.begin();
    ...
    for (stk::mesh::Bucket::size_type k = 0; k < length; ++k) {
      ...
      for (int i = 0; i < nDim; ++i) {
        ...
        for (int j = 0; j < nDim; ++j) {
          const double rateOfStrain =
            0.5 * (avgDudx[offSet + j] + avgDudx[nDim * j + i]);
      ...
      double v2 =
        std::min(cMu_ * fMu / 0.22, 2.0 / 3.0) * std::max(tke[k], 1.0e-16);
      v2 = std::max(v2, 2.0 / 3.0 * 1.0e-16);
      const double turbKE = std::max(tke[k], 1.0e-16);
      const double eps = std::max(tdr[k], 1.0e-16);
      double T =
        std::max(turbKE / eps, 6.0 * std::sqrt(visc[k] / (rho[k] * eps)));
      T = std::min(
        T,
        0.6 * turbKE / std::max(std::sqrt(6.0) * 0.22 * v2 * sijMag, 1.0e-12));
      T = std::min(T, 4.0);
      T = std::max(T, 1.0e-16);

      // tvisc[k] = cMu_ * fMu * rho[k] * tke[k] * tke[k] /
      // std::max(tdr[k],1.0e-16);
      // FIXME: Can't use this method until you change it so turbvisc is
      // calculated
      //        during SST model and during low mach model, since T relies on
      //        both the K, epsilon scalars and on the velocity field
