...
//--------------------------------------------------------------------------
void
ChienKEpsilonEquationSystem::register_interior_algorithm(stk::mesh::Part* part)
{
  // nothing to do here...
}
...
ChienKEpsilonEquationSystem::post_adapt_work()
{
  if (realm_.process_adaptivity()) {
    NaluEnv::self().naluOutputP0()
      << "--ChienKEpsilonEquationSystem::post_adapt_work()" << std::endl;
  }
}
...
ChienKEpsilonEquationSystem::update_and_clip()
{
  ...
  ScalarFieldType* turbViscosity = meta_data.get_field<ScalarFieldType>(
  ...
  for (stk::mesh::BucketVector::const_iterator ib = node_buckets.begin();
    ...
    for (stk::mesh::Bucket::size_type k = 0; k < length; ++k) {
      ...
      if ((tkeNew >= 0.0) && (tdrNew >= 0.0)) {
        ...
        // both negative; set k to small, tvisc to molecular visc and use
        // Prandtl/Kolm for tdr
        tke[k] = clipValue;
        tdr[k] = clipValue;
        const double fMu = 1.0 - std::exp(fMuExp * dp[k]);
        tvisc[k] = cMu * fMu * rho[k] * clipValue;
      } else if (tkeNew < 0.0) {
        // only tke is off; reset tvisc to molecular visc and compute new tke
        // appropriately
        tke[k] = clipValue;
        tdr[k] = tdrNew;
        const double fMu = 1.0 - std::exp(fMuExp * dp[k]);
        ...
        // only tdr if off; reset tvisc to molecular visc and compute new tdr
        // appropriately
        tdr[k] = clipValue;
        tke[k] = tkeNew;
        const double fMu = 1.0 - std::exp(fMuExp * dp[k]);
  ...
  if (realm_.debug()) {
    NaluEnv::self().naluOutputP0()
      << "Add KEps clipping diagnostic" << std::endl;
  }
}
...
ChienKEpsilonEquationSystem::compute_dplus_function()
{
  ...
  // model paramters FIXME: This shouldn't be an input option, needs to be
  ...
  for (stk::mesh::BucketVector::const_iterator ib = node_buckets.begin();
    ...
    for (stk::mesh::Bucket::size_type k = 0; k < length; ++k) {
      ...
      // dynamically for non-channel cases and not just at the wall, but
      // available everywhere based on which point is the closest to the wall
      // from that one...
