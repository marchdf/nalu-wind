...
SSTTAMSAveragesAlg::execute()
{
  ...
  if (meta.spatial_dimension() != 3) {
    throw std::runtime_error("SSTTAMSAveragesAlg only supported in 3D.");
  }
  ...
  nalu_ngp::run_entity_algorithm(
    ...
    sel, KOKKOS_LAMBDA(const Traits::MeshIndex& mi) {
      ...
      // Not in CDP as of now, in CDP k-eps, so might need to do something down
      // road
      ...
      for (int i = 0; i < nalu_ngp::NDimMax; ++i) {
        ...
        // FIXME: For future assessment about consistency between avgProd and
        // avgResAdeq
        // tij[i][i] -= 2.0/3.0 * beta.get(mi, 0) * density.get(mi, 0) *
        // tke.get(mi, 0);
      }
      ...
      if (tke.get(mi, 0) == 0.0)
        ...
        // FIXME: This may not be needing any more, requires further
        // investigation
        //       Used to correct near wall nodes issues with TKE that were
        //       resolved with improved SST clipping
