...
TurbViscSSTLRAlg::execute()
{
  ...
  nalu_ngp::run_entity_algorithm(
    ...
    KOKKOS_LAMBDA(const Traits::MeshIndex& meshIdx) {
      ...
      // FIXME: I need a better understanding of when to do this switch
      // tvisc.get(meshIdx, 0) = stk::math::if_then_else(aOne * sdr.get(meshIdx,
      // 0) >= sijMag * fTwo,
      //               aStar * density.get(meshIdx, 0) * tke.get(meshIdx, 0) /
      //               stk::math::max(sdr.get(meshIdx, 0), 1.e-8),
      //               density.get(meshIdx, 0) * aOne * tke.get(meshIdx, 0) /
      //               (sijMag * fTwo));
