...
// Class Definition
//==========================================================================
// ComputeSSTTAMSResAdequacyNodeAlgorithm - Metric Tensor
//==========================================================================
//--------------------------------------------------------------------------
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
ComputeSSTTAMSResAdequacyNodeAlgorithm::ComputeSSTTAMSResAdequacyNodeAlgorithm(
    ...
    CMdeg_(realm.get_turb_model_constant(TM_CMdeg))
{
  ...
  alphaNp1_ =
    metaData.get_field<ScalarFieldType>(stk::topology::NODE_RANK, "k_ratio");
  dudx_ =
...
ComputeSSTTAMSResAdequacyNodeAlgorithm::
  ~ComputeSSTTAMSResAdequacyNodeAlgorithm()
{
  tmpFile.close();
}
...
//--------------------------------------------------------------------------
void
ComputeSSTTAMSResAdequacyNodeAlgorithm::execute()
{
    ...
    for (stk::mesh::Bucket::size_type k = 0; k < length; ++k) {
      ...
      for (unsigned i = 0; i < nDim_; ++i) {
        ...
        for (unsigned j = 0; j < nDim_; ++j) {
          ...
          p_tauSGRS[i * nDim_ + j] =
            avgDudx[i * nDim_ + j] + avgDudx[j * nDim_ + i];
          ...
          for (unsigned l = 0; l < nDim_; ++l) {
            ...
            const double fluctDudx_jl =
              dudx[j * nDim_ + l] - avgDudx[j * nDim_ + l];
            const double fluctDudx_il =
              dudx[i * nDim_ + l] - avgDudx[i * nDim_ + l];
            p_tauSGET[i * nDim_ + j] +=
      ...
      for (unsigned i = 0; i < nDim_; ++i)
        for (unsigned j = 0; j < nDim_; ++j)
          p_tau[i * nDim_ + j] =
            p_tauSGRS[i * nDim_ + j] + p_tauSGET[i * nDim_ + j] -
      ...
      for (unsigned i = 0; i < nDim_; ++i) {
        for (unsigned j = 0; j < nDim_; ++j) {
          for (unsigned l = 0; l < nDim_; ++l) {
            p_Psgs[i * nDim_ + j] +=
              p_tau[i * nDim_ + l] * dudx[l * nDim_ + j] +
